...
function ProcessDetail({ id, setDetail, detail, setOpen }) {
  const [loading, setLoading] = useState(true);
  const [process, setProcess] = useState({});
  const { numero, descricao, assunto, interessados, entrada } = process;
  ...
  return loading ? (
    ...
    <S.ProcessDetailWrapper>
      ...
      <div className="top">
        ...
        <div className="top_content">
          <div className="process-number">
            ...
            <Typography variant="h2">{numero}</Typography>
          </div>
          <div>
            <Typography variant="h3">Data</Typography>
            <Typography variant="h2">{entrada}</Typography>
          </div>
          <div className="subject">
            <Typography variant="h3">Assunto</Typography>
            <Typography variant="h2">{assunto}</Typography>
          </div>
        </div>
      </div>
      ...
      <div className="buttons">
        ...
        <Button variant="outlined" onClick={() => setOpen(true)}>Editar</Button>
      </div>
    </S.ProcessDetailWrapper>
  );
}
